...
public static int maxResultInefficent(int[] nums, int k) {
    return maxResult(nums,k,0);
}

 public static int maxResult(int[] nums, int k,int cp) {
     if(cp>nums.length-1){
     ...
     for(int i=1;i<=k;i++){
        maxScore=Integer.max(maxScore,maxResult(nums,k,cp+i));//1 
     }
     return maxScore+ nums[cp];
 }
